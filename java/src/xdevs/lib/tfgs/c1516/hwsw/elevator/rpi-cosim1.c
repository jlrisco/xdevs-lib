...
const unsigned int dataRPin[4] = {21,20,16,12}; // {S1,S2,S3,S4}
...
void free_all_gpio(void){
	...
	gpio_free(dataSPin[4]);
	gpio_free(dataSPin[5]);
}
...
static ssize_t elevator_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
    ...
    /* Charges the C value */
    sprintf(kbuff, "%i%i%i%i", gpio_get_value(dataRPin[3]), gpio_get_value(dataRPin[2]), gpio_get_value(dataRPin[1]), gpio_get_value(dataRPin[0]));
